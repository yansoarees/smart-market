...
function Loja() {
  ...
  function getQtdNoCarrinho(idProduto) {
    return carrinho
      ...
      .reduce((acc, item) => acc + parseFloat(item.quantidade), 0);
  }
      ...
      try {
        ...
        const produtosProntos = dados.map((produto) => {
          ...
          if (nomeLower.includes("ovo")) {
            opcoes = [
              { id: "unidade", label: "Unidade", preco: 0.7 },
            ...
            opcoes = [
              { id: "50ml", label: "50ML", preco: 2.49 },
              { id: "150ml", label: "150ML", preco: 3.99 },
              { id: "180ml", label: "180ML", preco: 4.49 },
              { id: "200ml", label: "200ML", preco: 5.75 },
              { id: "250ml", label: "250ML", preco: 7.49 },
              { id: "300ml", label: "300ML", preco: 8.99 },
            ];
          } else if (nomeLower.includes("prato")) {
            opcoes = [
              { id: "15cm", label: "15CM", preco: 1.49 },
              { id: "18cm", label: "18CM", preco: 2.39 },
              { id: "21cm", label: "21CM", preco: 2.99 },
            ];
          }

          let precoNormal = converterParaNumero(produto.preco);
          let isPromo = produto.promocao === 1 || produto.promocao === true;
          let precoPromo = isPromo
            ? converterParaNumero(produto.preco_promocao)
            : precoNormal;
          let qtdPromo =
            isPromo && produto.qtd_promocao
              ? parseInt(produto.qtd_promocao)
              : 1;

          let mensagemPromo =
            isPromo && qtdPromo > 1
              ? `A partir de ${qtdPromo} un → R$ ${formatarDinheiro(precoPromo)} cada`
              : "";
          ...
          if (
            ...
            try {
              variacoesLidas = JSON.parse(produto.variacoes);
            } catch (e) {}
          }
  ...
  function handleAdicionarClick(produto) {
    ...
    if (
      catProd === "frios" ||
      nomeProd.includes("queijo") ||
      nomeProd.includes("presunto") ||
      temOpcoes ||
      temVariacoesDB
    ) {
      ...
      if (qtdAtual + 1 > produto.estoque) {
        alert(
          `Oops! Temos apenas ${produto.estoque} unidades de ${produto.nome} no estoque.`,
        );
        return;
      }
  ...
  function confirmarAdicaoAoCarrinho(produto, qtd, detalhes, precoFinalizado) {
    ...
    if (itemExistente) {
      const novaQtd =
        (produto.categoria || "").toLowerCase() === "frios" ||
        (produto.nome || "").toLowerCase().includes("queijo") ||
        (produto.nome || "").toLowerCase().includes("presunto")
          ? parseFloat(itemExistente.quantidade) + parseFloat(qtd)
          : itemExistente.quantidade + qtd;

      setCarrinho(
        carrinho.map((item) =>
          item.idUnico === idUnico ? { ...item, quantidade: novaQtd } : item,
        ),
      );
    } else {
      setCarrinho([
        ...carrinho,
        {
          ...
          infoAdicional: detalhes ? detalhes.info : "",
        },
      ]);
    }
  ...
  function alterarQuantidadeCart(idUnico, delta) {
    ...
    if (delta > 0) {
      if (itemExistente.estoqueSaborLimit !== undefined) {
        if (
          itemExistente.quantidade + delta >
          itemExistente.estoqueSaborLimit
        ) {
          alert(
            `Limite atingido! Temos apenas ${itemExistente.estoqueSaborLimit} do sabor ${itemExistente.infoAdicional}.`,
          );
          return;
        }
      } else {
        ...
        if (qtdTotalDesseProduto + delta > itemExistente.estoque) {
          alert(
            `Limite de estoque atingido! Temos apenas ${itemExistente.estoque} disponíveis.`,
          );
          return;
        }
      }
  ...
  // Função responsável por calcular o subtotal do item
  // Aplica as regras de negócio de precificação (atacado/varejo e produtos pesados em KG)
  function calcularPrecoItem(item) {
    const nomeLower = (item.nome || "").toLowerCase();

    if (
      (item.categoria || "").toLowerCase() === "frios" ||
      nomeLower.includes("queijo") ||
      nomeLower.includes("presunto") ||
      item.infoAdicional
    ) {
  ...
  const taxa = tipoEntrega === "Entrega" ? 5.0 : 0;
  const total = subtotal + taxa;

  const totalPagoMultiplo = pagamentosMultiplos.reduce(
    (acc, p) => acc + converterParaNumero(p.valor),
    0,
  );
  ...
  function avancarParaPagamento() {
    ...
    if (palavrasNome.length < 2)
      return setErroValidacao("Por favor, informe seu nome e sobrenome.");
    if (/\d/.test(nomeLimpo))
      return setErroValidacao("O nome não pode conter números.");

    for (let palavra of palavrasNome) {
      if (palavra.length < 2)
        return setErroValidacao("Seu nome ou sobrenome está muito curto.");
      if (!/[aeiouyáéíóúãõâêîôû]/i.test(palavra))
        return setErroValidacao("Por favor, digite um nome real válido.");
    }
    ...
    if (!dddsValidos.includes(ddd))
      return setErroValidacao(
        `O DDD (${ddd}) não existe. Informe um DDD válido, ex: (81).`,
      );

    const celularRegex = /^\([1-9]{2}\)\s9\d{4}-\d{4}$/;
    if (!celularRegex.test(telefone))
      return setErroValidacao(
        "Informe um celular válido no formato (XX) 9XXXX-XXXX.",
      );
  ...
  async function enviarPedido() {
    ...
    if (modoPagamento === "dividido" && valorRestante > 0) {
      setErroValidacao(
        `Ainda falta pagar R$ ${formatarDinheiro(valorRestante)}!`,
      );
      return;
    }
    ...
    if (modoPagamento === "unico") {
      const troco =
        converterParaNumero(trocoPara) > total
          ? converterParaNumero(trocoPara)
          : 0;
      pagamentoStrDB =
        metodoUnico === "Dinheiro" && troco > 0
          ? `Dinheiro (Troco para R$ ${formatarDinheiro(troco)})`
          : metodoUnico;
    ...
    const dadosPedido = {
      ...
      endereco:
        tipoEntrega === "Entrega"
          ? `${endereco.rua}, ${endereco.numero} - ${endereco.bairro} (Ref: ${endereco.ref})`
          : "Retirada",
    ...
    carrinho.forEach((item) => {
      ...
      const medida =
        (item.categoria || "").toLowerCase() === "frios" ||
        (item.nome || "").toLowerCase().includes("queijo") ||
        (item.nome || "").toLowerCase().includes("presunto")
          ? "kg"
          : "x";
  ...
  function addLinhaPagamento() {
    setPagamentosMultiplos([
      ...pagamentosMultiplos,
      { id: Date.now(), metodo: "Cartão", valor: "" },
    ]);
  }
  ...
  const produtosAgrupados = produtos
    .filter((p) => {
      const statusAtivo =
        p.ativo === 1 || p.ativo === true || p.ativo === undefined;
      return (
        statusAtivo &&
        p.estoque > 0 &&
    ...
    .reduce((grupos, produto) => {
      ...
      const isAbaPromocao =
        produto.isPromo || catLower === "promoção" || catLower === "promocao";
      ...
      <main>
            ...
            <div className="search" style={{ flex: 1, position: "relative" }}>
              ...
              >
                <path
                  ...
                  strokeLinecap="round"
                />
                <circle cx="11" cy="11" r="6" stroke="#fff" strokeWidth="1.5" />
              </svg>
              <input
                type="search"
            ...
            <div style={{ position: "relative" }}>
              ...
              >
                ...
                <svg
                  ...
                  strokeWidth={2}
                  style={{
                    transform: menuCategoriasAberto ? "rotate(180deg)" : "none",
                    transition: "0.3s",
                  }}
                >
                  <path
          ...
          <div id="produtos">
            {loading ? (
              ...
              categoriasOrdenadas.map((categoria) => (
                <div
                  key={categoria}
                  id={`secao-${formatarNome(categoria)}`}
                  style={{
                    width: "100%",
                    marginBottom: "30px",
          ...
          >
            ...
            >
              ...
              >
                ...
                {(produtoConfigurando.categoria || "").toLowerCase() ===
                  "frios" ||
                (produtoConfigurando.nome || "")
                  .toLowerCase()
                  .includes("queijo") ||
                (produtoConfigurando.nome || "")
                  ...
                  <div style={{ marginBottom: "25px" }}>
                    ...
                    <input
                      ...
                      value={configPeso}
                      onChange={(e) => setConfigPeso(e.target.value)}
                      style={{
                        width: "100%",
                        padding: "12px",
                        borderRadius: "6px",
                  ...
                  <>
                          ...
                          {produtoConfigurando.opcoes.map((opcao) => {
                            ...
                            return (
                              <div
                                ...
                                style={{
                                  border: `2px solid ${selecionado ? "#00b853" : "#eee"}`,
                                  borderRadius: "8px",
                                  padding: "15px",
                                  textAlign: "center",
                                  cursor: "pointer",
                              ...
                              >
                                ...
                                >
                                  R$ {formatarDinheiro(opcao.preco)}
                                </div>
                              </div>
                            );
                          })}
                    ...
                    {produtoConfigurando.variacoes &&
                      produtoConfigurando.variacoes.length > 0 && (
                        <div
                          style={{
                            marginBottom: "20px",
                            maxHeight: "180px",
                            ...
                            return (
                              <div
                                ...
                                style={{
                                  ...
                                  marginBottom: "8px",
                                  padding: "8px",
                                  background: esgotadoSabor
                                    ? "#ffebee"
                                    : "#f5f5f5",
                                  borderRadius: "5px",
                                  opacity: esgotadoSabor ? 0.6 : 1,
                                }}
                              >
                                <span
                                  style={{
                                    fontWeight: "500",
                                    color: esgotadoSabor ? "#c0392b" : "#333",
                                  }}
                                >
                                  {varDB.nome}{" "}
                                  <span
                    ...
                    {!(
                      ...
                      <div style={{ marginBottom: "25px" }}>
                        ...
                        <input
                          ...
                          min="1"
                          value={configQtd}
                          onChange={(e) => setConfigQtd(e.target.value)}
                          style={{
                            width: "100%",
                            padding: "12px",
                            borderRadius: "5px",
                ...
                <button
                  onClick={() => {
                    let info = "";
                    let tipo = "padrao";
                    let valor = configPeso;
                    let quantidadeFinal = parseInt(configQtd) || 1;
                    let precoParaCarrinho = produtoConfigurando.isPromo
                      ? produtoConfigurando.precoPromo
                      : produtoConfigurando.precoNormal;

                    const cat = (
                      produtoConfigurando.categoria || ""
                    ).toLowerCase();
                    ...
                    if (
                      ...
                      if (isNaN(pesoKg) || pesoKg <= 0)
                        return alert(
                          "Por favor, digite um peso válido! Ex: 0.250",
                        );
                      tipo = "peso";
                      info = `${pesoKg}kg`;
                      ...
                      if (
                        qtdAtual + quantidadeFinal >
                        produtoConfigurando.estoque
                      ) {
                        alert(
                          `Oops! Temos apenas ${produtoConfigurando.estoque} unidades no estoque. Você já tem ${qtdAtual} no carrinho.`,
                        );
                        return;
                      }
                      confirmarAdicaoAoCarrinho(
                      ...
                      produtoConfigurando.opcoes &&
                      produtoConfigurando.opcoes.length > 0
                    ) {
                      tipo = "opcoes";
                      info = opcaoSelecionada ? opcaoSelecionada.label : "";
                      precoParaCarrinho = opcaoSelecionada
                      ...
                      tipo = "sabores";
                      const selecionados = Object.entries(configSabores).filter(
                        ([s, q]) => parseInt(q) > 0,
                      );
                      if (selecionados.length === 0)
                        return alert(
                          "Escolha pelo menos 1 sabor e informe a quantidade!",
                        );
                      ...
                      for (let [saborNome, qtdStr] of selecionados) {
                        ...
                        const varDB = produtoConfigurando.variacoes.find(
                          (v) => v.nome === saborNome,
                        );
                        ...
                        const itemExistenteIndex = novoCarrinho.findIndex(
                          (c) => c.idUnico === idUnicoSabor,
                        );
                        const qtdAtualSabor =
                          itemExistenteIndex >= 0
                            ? novoCarrinho[itemExistenteIndex].quantidade
                            : 0;

                        if (qtdAtualSabor + qtdSabor > maxSabor) {
                          alert(
                            `Você só pode adicionar mais ${maxSabor - qtdAtualSabor} do sabor ${saborNome}. Temos ${maxSabor} disponíveis.`,
                          );
                          return;
                        }
        ...
        <aside className={`pedido ${carrinhoAberto ? "aberto" : ""}`}>
          ...
          <div id="pedido-box">
            ...
            <div id="lista" style={{ maxHeight: "300px", overflowY: "auto" }}>
              {carrinho.length === 0 ? (
                ...
                carrinho.map((item) => {
                  ...
                  const aplicouPromo =
                    item.isPromo && item.quantidade >= item.qtdPromo;
                  const valorUnitarioAtual = aplicouPromo
                    ? item.precoPromo
                    : item.precoNormal || item.precoVenda;
                  const strMedida =
                    (item.categoria || "").toLowerCase() === "frios" ||
                    (item.nome || "").toLowerCase().includes("queijo") ||
                    (item.nome || "").toLowerCase().includes("presunto")
                  ...
                  return (
                    ...
                    >
                      ...
                      >
                        <span style={{ fontWeight: "500" }}>
                          ...
                          >
                            {item.infoAdicional
                              ? `${item.infoAdicional} • `
                              : ""}
                            {item.quantidade} {strMedida} × R${" "}
                            {formatarDinheiro(valorUnitarioAtual)} ={" "}
                            <strong style={{ color: "#1a3b5c" }}>
                              R$ {formatarDinheiro(precoItemTotal)}
                            </strong>
                          </span>
                          {aplicouPromo && (
                            <span
                        ...
                        <span style={{ fontWeight: "bold", color: "#1a3b5c" }}>
                          R$ {formatarDinheiro(precoItemTotal)}
                        </span>
                      </div>
                      <div
                        ...
                        {(item.categoria || "").toLowerCase() !== "frios" &&
                          ...
                          !(item.nome || "")
                            ...
                            >
                              <button
                                onClick={() =>
                                  alterarQuantidadeCart(item.idUnico, -1)
                                }
                                style={{
                                  background: "white",
                                  color: "#e74c3c",
            ...
            <div
              className="line"
              style={{ height: "1px", background: "#ccc", margin: "15px 0" }}
            ></div>
            ...
            <div className="summary">
              ...
              >
                <div>Subtotal</div>
                <div>R$ {formatarDinheiro(subtotal)}</div>
              </div>
              <div
                className="summary-row"
                style={{
              ...
              >
                <div>Taxa</div>
                <div>R$ {formatarDinheiro(taxa)}</div>
              </div>
              <div
                className="summary-row"
                style={{
              ...
              >
                ...
                <div>
                  <strong>R$ {formatarDinheiro(total)}</strong>
                </div>
              </div>
            </div>

            <div
              className="btns-final"
              style={{
                display: "flex",
                flexDirection: "column",
                gap: "10px",
            ...
            >
              ...
              <button
                ...
                style={{
                  background: carrinho.length === 0 ? "#ccc" : "#00b853",
                  color: "white",
                  border: "none",
                  padding: "15px",
                  borderRadius: "5px",
        ...
        >
          ...
          {etapaModal === "pagamento" && (
            ...
            >
              ...
              >
                <h2 style={{ margin: 0, color: "#1a3b5c" }}>
                  Pagamento e Entrega
                </h2>
                <button
              ...
              >
                <div style={{ fontWeight: "bold", marginBottom: "5px" }}>
                  Total a pagar: R$ {formatarDinheiro(total)}
                </div>
              </div>
                ...
                >
                  <button
                    ...
                    style={{
                      ...
                      fontWeight: "bold",
                      background:
                        modoPagamento === "unico" ? "white" : "transparent",
                      color: modoPagamento === "unico" ? "#1a3b5c" : "#777",
                      boxShadow:
                        modoPagamento === "unico"
                  ...
                  <button
                    onClick={() => setModoPagamento("dividido")}
                    style={{
                      flex: 1,
                      padding: "10px",
                      borderRadius: "4px",
                      ...
                      fontWeight: "bold",
                      background:
                        modoPagamento === "dividido" ? "white" : "transparent",
                      color: modoPagamento === "dividido" ? "#1a3b5c" : "#777",
                      boxShadow:
                ...
                <div style={{ marginBottom: "20px" }}>
                  ...
                  >
                    ...
                    >
                      <input
                        type="radio"
                        checked={metodoUnico === "Dinheiro"}
                        onChange={() => setMetodoUnico("Dinheiro")}
                      />{" "}
                      Dinheiro
                    </label>
                    <label
                    ...
                    >
                      <input
                        type="radio"
                        checked={metodoUnico === "Cartão"}
                        onChange={() => setMetodoUnico("Cartão")}
                      />{" "}
                      Cartão
                    </label>
                    <label
                    ...
                    >
                      <input
                        type="radio"
                        checked={metodoUnico === "Pix"}
                        onChange={() => setMetodoUnico("Pix")}
                      />{" "}
                      Pix
                    </label>
                  </div>
                  ...
                  {pagamentosMultiplos.map((pag, index) => (
                    ...
                    >
                      <select
                        value={pag.metodo}
                        onChange={(e) =>
                          updateLinhaPagamento(pag.id, "metodo", e.target.value)
                        }
                        style={{
                          flex: 1,
                          padding: "10px",
                      ...
                      <input
                        ...
                        value={pag.valor}
                        onChange={(e) =>
                          updateLinhaPagamento(pag.id, "valor", e.target.value)
                        }
                        style={{
                          flex: 1,
                          padding: "10px",
                      ...
                      {pagamentosMultiplos.length > 1 && (
                        <button
                          onClick={() => removeLinhaPagamento(pag.id)}
                          style={{
                            background: "#e74c3c",
                            color: "white",
                            border: "none",
                  ...
                  >
                    <div
                      style={{
                        display: "flex",
                        justifyContent: "space-between",
                        color: valorRestante > 0 ? "#e74c3c" : "#2ecc71",
                        fontWeight: "bold",
                      }}
                    >
                      <span>
                      ...
                      <span>
                        R$ {formatarDinheiro(Math.abs(valorRestante))}
                      </span>
                    </div>
                  </div>
                </div>
                ...
                {tipoEntrega === "Entrega" && (
                  ...
                  >
                    ...
                    <input
                      ...
                      value={endereco.ref}
                      onChange={(e) =>
                        setEndereco({ ...endereco, ref: e.target.value })
                      }
                      style={{
                        padding: "10px",
                        border: "1px solid #ccc",
