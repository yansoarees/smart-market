...
export default function Admin() {
  ...
  const [formProduto, setFormProduto] = useState({
    nome: '', categoria: '', preco: '', promocao: false, preco_promocao: '', qtd_promocao: '1', imagem: null
  });
  ...
  const dadosFaturamentoSemana = [
    { name: 'Seg', valor: 340 }, { name: 'Ter', valor: 450 }, { name: 'Qua', valor: 310 },
    { name: 'Qui', valor: 680 }, { name: 'Sex', valor: 850 }, { name: 'Sáb', valor: 1250 }, { name: 'Dom', valor: 1050 }
  ];

  const clientesAgrupados = pedidos.reduce((acc, p) => {
    if (p.status !== 'Cancelado') {
      if (!acc[p.telefone]) {
        acc[p.telefone] = { nome: p.nomeCliente, telefone: p.telefone, totalGasto: 0, qtdPedidos: 0 };
      }
  ...
  function handleAbrirNovo() {
    ...
    setFormProduto({ nome: '', categoria: '', preco: '', promocao: false, preco_promocao: '', qtd_promocao: '1', imagem: null });
    setModalAberto(true);
  }
  ...
  async function handleSalvarProduto(e) {
    ...
    try {
      ...
      if (resposta.ok) {
        ...
        alert("Ops! O servidor negou o salvamento."); 
      }
    } catch (erro) { 
    ...
    try {
      ...
      if (resposta.ok) {
        ...
        if (novoStatus === 'Em Preparo' || novoStatus === 'Saiu para Entrega' || novoStatus === 'Pronto para Retirada') {
          const foneLimpo = pedido.telefone.replace(/\D/g, ''); 
          let texto = '';
          
          if (novoStatus === 'Em Preparo') {
            texto = `Olá *${pedido.nomeCliente}*! 🛒\nPassando para avisar que seu pedido #${pedido.id} já está *Em Preparo* pela nossa equipe! Em breve sairá para entrega.`;
          } else if (novoStatus === 'Saiu para Entrega') {
            texto = `Boas notícias, *${pedido.nomeCliente}*! 🛵💨\nSeu pedido #${pedido.id} acabou de *Sair para Entrega*! Fique de olho, estamos chegando!`;
          } else if (novoStatus === 'Pronto para Retirada') {
            texto = `Olá *${pedido.nomeCliente}*! 🛍️\nBoas notícias: seu pedido #${pedido.id} já está *Pronto para Retirada*! Já pode vir buscar no nosso balcão.`;
          }
  ...
  function handleAbrirDetalhesPedido(pedido) {
    let itensTraduzidos = [];
    try { itensTraduzidos = typeof pedido.itens === 'string' ? JSON.parse(pedido.itens) : pedido.itens; } 
    catch (error) { itensTraduzidos = []; }
    setPedidoSelecionado({ ...pedido, itensLista: itensTraduzidos });
  }

  // =========================================================
  // ★ FUNÇÕES DE MÁGICA DA IMAGEM PARA O ADMIN ★
  // =========================================================
    ...
    <div className="admin-layout" translate="no">
      ...
      <div className="sidebar">
        ...
        <div className="sidebar-menu">
          <div className={`menu-item ${abaAtiva === 'dashboard' ? 'ativo' : ''}`} onClick={() => setAbaAtiva('dashboard')}>
            <IconDashboard /> Visão Geral
          </div>
          <div className={`menu-item ${abaAtiva === 'produtos' ? 'ativo' : ''}`} onClick={() => setAbaAtiva('produtos')}>
            <IconBox /> Estoque
          </div>
          <div className={`menu-item ${abaAtiva === 'pedidos' ? 'ativo' : ''}`} onClick={() => setAbaAtiva('pedidos')}>
            <IconClipboard /> Pedidos
          </div>
          <div className={`menu-item ${abaAtiva === 'clientes' ? 'ativo' : ''}`} onClick={() => setAbaAtiva('clientes')}>
            <IconUsers /> Clientes
          </div>
        </div>
      </div>
      ...
      <div className="main-content">
        ...
        <div className="content-area">
          <h1 className="page-title">
            {abaAtiva === 'produtos' ? 'Estoque e Produtos' : 
             abaAtiva === 'dashboard' ? 'Visão Geral do Negócio' : 
             abaAtiva === 'clientes' ? 'Carteira de Clientes' : 'Gestão de Pedidos'}
          </h1>
          ...
          {abaAtiva === 'dashboard' && (
            <>
              {loading ? (
                <div style={{ padding: '40px', textAlign: 'center', color: '#64748b' }}>Carregando métricas...</div>
              ) : (
                <>
                  <div className="metrics-grid">
                    <div className="metric-card">
                      <div className="metric-header">
                        ...
                        <div className="icon-box" style={{ background: '#dcfce7', color: '#059669' }}><IconClipboard /></div>
                      </div>
                      <p className="metric-value">R$ {faturamentoTotal.toFixed(2).replace('.', ',')}</p>
                      <p className="metric-trend"><IconTrendingUp /> Em tempo real</p>
                    </div>
                    <div className="metric-card">
                      <div className="metric-header">
                        <h3>Total de Pedidos</h3>
                        <div className="icon-box" style={{ background: '#e0e7ff', color: '#4f46e5' }}><IconBox /></div>
                      </div>
                      <p className="metric-value">{pedidos.length}</p>
                      <p className="metric-trend neutral">{pedidos.filter(p => p.status === 'Pendente').length} aguardando análise</p>
                    </div>
                    <div className="metric-card">
                      <div className="metric-header">
                        <h3>Estoque Ativo</h3>
                        <div className="icon-box" style={{ background: '#fef3c7', color: '#d97706' }}><IconDashboard /></div>
                      </div>
                      <p className="metric-value">{produtos.length}</p>
                      <p className="metric-trend neutral">{produtosEmPromocao} em promoção</p>
                    </div>
                    <div className="metric-card">
                      <div className="metric-header">
                        <h3>Ticket Médio</h3>
                        <div className="icon-box" style={{ background: '#fae8ff', color: '#c026d3' }}><IconClipboard /></div>
                      </div>
                      <p className="metric-value">R$ {ticketMedioPedidos.toFixed(2).replace('.', ',')}</p>
                      <p className="metric-trend neutral">Por pedido fechado</p>
                    </div>
                  </div>

                  <div className="charts-row">
                    <div className="chart-container">
                      <h3 className="chart-title">Faturamento Semanal <span className="chart-subtitle">*Ilustrativo</span></h3>
          ...
          {abaAtiva === 'produtos' && (
            <div className="table-container">
              ...
              {loading ? (
                <div style={{ padding: '40px', textAlign: 'center', color: '#64748b' }}>Sincronizando banco...</div>
              ) : (
                <div className="table-wrapper">
                  <table>
                    <thead>
                      <tr><th>Produto</th><th>Preço Base</th><th>Status</th><th style={{ textAlign: 'right' }}>Ações</th></tr>
                    </thead>
                    <tbody>
                      {produtos.map(prod => {
                        const emPromocao = prod.promocao === 1 || prod.promocao === true;
                        
                        // ★ A MÁGICA DA FOTO ANTIGA NO ADMIN ★
                        const catFormatada = formatarNome(prod.categoria);
                        const nomeFormatado = formatarNome(prod.nome);
                        const urlDaImagem = prod.imagem 
                          ? `http://127.0.0.1:3001${prod.imagem}` 
                          : `/img/${catFormatada}/${nomeFormatado}.png`;
                        
                        return (
                          <tr key={prod.id}>
                            <td>
                              <div className="td-title">
                                {/* O onError aciona o truque de testar .jpg, .jpeg, etc */}
                                <img 
                                  src={urlDaImagem} 
                                  alt={prod.nome} 
                                ...
                                <div>
                                  <div>{prod.nome}</div>
                                  <div className="td-subtitle">{prod.categoria}</div>
                                </div>
                              </div>
                            </td>
                            <td>
                              {emPromocao && prod.preco_promocao ? (
                                <>
                                  <span className="preco-antigo">R$ {Number(prod.preco).toFixed(2).replace('.', ',')}</span>
                                  <span className="preco-novo">R$ {Number(prod.preco_promocao).toFixed(2).replace('.', ',')}</span>
                                  {prod.qtd_promocao > 1 && (<div className="promo-qtd">A partir de {prod.qtd_promocao} un</div>)}
                                </>
                              ) : (<span className="preco-normal">R$ {Number(prod.preco).toFixed(2).replace('.', ',')}</span>)}
                            </td>
                            <td>
                              <span className={`badge ${emPromocao ? 'promo' : 'ativo'}`}>
                                <span style={{ width: 6, height: 6, borderRadius: '50%', background: emPromocao ? '#dc2626' : '#166534' }}></span>{emPromocao ? 'Em Promoção' : 'Ativo'}
                              </span>
                            </td>
                            <td>
          ...
          {abaAtiva === 'pedidos' && (
             <div className="table-container">
               <div className="table-header-actions">
                  <h3 style={{ margin: 0, color: '#0f172a', fontSize: '16px', fontWeight: 700 }}>Acompanhamento</h3>
                </div>
                {loading ? ( 
                  <div style={{ padding: '40px', textAlign: 'center', color: '#64748b' }}>Carregando pedidos...</div> 
                ) : (
                  <div className="table-wrapper">
                    <table>
                      <thead>
                        <tr><th>Pedido</th><th>Cliente</th><th>Modalidade</th><th>Total</th><th>Status</th><th style={{ textAlign: 'right' }}>Ações</th></tr>
                      </thead>
                      <tbody>
                        {pedidos.map(pedido => {
                          let classeStatus = "status-Pendente";
                          if (pedido.status === "Em Preparo") classeStatus = "status-Preparo";
                          if (pedido.status === "Saiu para Entrega") classeStatus = "status-Entrega";
                          if (pedido.status === "Pronto para Retirada") classeStatus = "status-Retirada";
                          if (pedido.status === "Concluído") classeStatus = "status-Concluido";
                          ...
                          return (
                            <tr key={pedido.id}>
                              ...
                              <td>
                                <select 
                                  className={`select-status ${classeStatus}`} 
                                  value={pedido.status} 
                                  onChange={(e) => handleAtualizarStatusPedido(pedido, e.target.value)}
                                >
                              ...
                              <td>
                                <div className="actions-flex" style={{ justifyContent: 'flex-end' }}>
                                  <button className="btn-icon view" onClick={() => handleAbrirDetalhesPedido(pedido)}><IconEye /> Ver</button>
                                </div>
                              </td>
                            </tr>
                          );
          ...
          {abaAtiva === 'clientes' && (
             <div className="table-container">
               <div className="table-header-actions">
                  <h3 style={{ margin: 0, color: '#0f172a', fontSize: '16px', fontWeight: 700 }}>Ranking de Clientes (CRM)</h3>
                </div>
                {loading ? ( 
                  <div style={{ padding: '40px', textAlign: 'center', color: '#64748b' }}>Analisando base...</div> 
                ) : (
                  <div className="table-wrapper">
                    <table>
                      <thead>
                        <tr><th>Cliente</th><th>Telefone</th><th>Pedidos</th><th>Total Gasto</th></tr>
                      </thead>
                      <tbody>
                        {listaClientes.map((cliente, index) => (
                          <tr key={cliente.telefone}>
                            <td>
                              <div className="td-title" style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                                {cliente.nome} {index < 3 && <span className="badge vip">⭐ VIP</span>}
                              </div>
                            </td>
                            <td className="td-subtitle">{cliente.telefone}</td>
                            <td><span className="badge ativo">{cliente.qtdPedidos}</span></td>
          ...
          <div className="modal-overlay">
            <div className="modal-box">
              <div className="modal-header">
                <h3>{produtoEditando ? 'Configurar Produto' : 'Novo Produto'}</h3>
                <button className="btn-fechar" onClick={() => { setModalAberto(false); setProdutoEditando(null); }}><svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
              </div>
                  ...
                  <div className="form-group">
                    ...
                    <div className="file-upload-wrapper">
                      <input 
                        ...
                        onChange={(e) => setFormProduto({...formProduto, imagem: e.target.files[0]})} 
                      />
                      <div style={{ color: '#4f46e5', marginBottom: '4px' }}><IconImage /></div>
                      <div className="file-upload-text">
                        {formProduto.imagem ? formProduto.imagem.name : 'Clique para selecionar uma imagem (JPG, PNG)'}
                      </div>
                    </div>
                  </div>
              ...
              <div className="modal-footer">
                <button type="button" className="btn-cancelar" onClick={() => { setModalAberto(false); setProdutoEditando(null); }}>Cancelar</button>
                <button type="submit" form="form-produto" className="btn-salvar">Salvar Produto</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div className="modal-overlay">
            <div className="modal-box">
              <div className="modal-header">
                ...
                <button className="btn-fechar" onClick={() => setPedidoSelecionado(null)}><svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg></button>
              </div>
              <div className="modal-body">
                <div className="resumo-pedido">
                  <div className="resumo-linha"><span className="resumo-label">Cliente:</span><span className="resumo-dado">{pedidoSelecionado.nomeCliente}</span></div>
                  <div className="resumo-linha"><span className="resumo-label">Contato:</span><span className="resumo-dado">{pedidoSelecionado.telefone}</span></div>
                  <div className="resumo-linha"><span className="resumo-label">Modalidade:</span><span className="resumo-dado">{pedidoSelecionado.formaEntrega}</span></div>
                  <div className="resumo-linha"><span className="resumo-label">Pagamento:</span><span className="resumo-dado" style={{color: '#059669'}}>{pedidoSelecionado.formaPagamento}</span></div>
                </div>
                <h4 style={{ margin: '0 0 16px 0', color: '#374151', fontSize: '15px' }}>Itens da Compra</h4>
                <ul className="lista-itens">
                  {pedidoSelecionado.itensLista && pedidoSelecionado.itensLista.length > 0 ? (
                    pedidoSelecionado.itensLista.map((item, index) => (
                      <li key={index}>
                        <div><span style={{fontWeight: 600, color: '#0f172a', fontSize: '14px'}}>{item.nome} {item.infoAdicional ? `(${item.infoAdicional})` : ''}</span><span style={{display: 'block', fontSize: '13px', color: '#64748b'}}>Qtd: {item.quantidade} {item.tipo || 'un'}</span></div>
                        <span style={{fontWeight: 700, color: '#059669'}}>R$ {Number(item.preco * item.quantidade).toFixed(2).replace('.', ',')}</span>
                      </li>
                    ))
                  ) : (<li style={{ color: '#ef4444' }}>Nenhum item listado.</li>)}
                </ul>
              </div>
              <div className="modal-footer" style={{ justifyContent: 'space-between', alignItems: 'center' }}>
                <div style={{ fontSize: '18px', fontWeight: '800', color: '#0f172a' }}>Total: R$ {Number(pedidoSelecionado.total).toFixed(2).replace('.', ',')}</div>
                <button type="button" className="btn-salvar" onClick={() => setPedidoSelecionado(null)}>Fechar</button>
              </div>
            </div>
          </div>
        )}
